...
void Encriptador::encriptar(){
    ...
    if (k_priv!=0){
        ...
        fstream archivo1("data.txt", ios::out);
        if (!archivo1) {
        ...
        archivo1.close();
        fstream archivo2("encriptadooo.txt", ios::in);
        if (!archivo2) {
            cerr << "no se pudo abrir archivo xd" << endl;
        }
        archivo2<<textoo1;
...
void Encriptador::desencriptar(int c_priv){
    if (c_priv == k_priv){
        //f_processing(k_priv);
        fstream archivo1("encriptadooo.txt", ios::in | ios::out);
        if (!archivo1) {
            cerr << "no se pudo abrir archivo xd" << endl;
        }
        fstream archivo2("desencriptadooo.txt", ios::in | ios::out);
        if (!archivo2) {
            cerr << "no se pudo abrir archivo xd" << endl;
        }
        while (archivo1.get(caracter)){
